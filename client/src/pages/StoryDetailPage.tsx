import { ArrowLeft, Share2, Heart, Calendar, Package, Gift, Camera } from "lucide-react";
...
const storiesData: Record<string, {
  ...
  memoryBox: { title: string; story: string; item: string };
  products: { name: string; benefit: string; desc: string }[];
}> = {
  "1": {
    month: "1월",
    theme: "새해 건강 기원",
    ...
    products: [
      { name: "6년근 홍삼정과 150g", benefit: "면역력 증진", desc: "금산 GAP인증 농가 직거래, 전통 꿀 절임 방식" },
      { name: "국산 대추칩 80g", benefit: "혈액순환 개선", desc: "경북 경산 대추, 무첨가 자연건조" },
      { name: "유기농 호두 100g", benefit: "두뇌 건강", desc: "국산 유기농 인증, 저온 로스팅" },
      { name: "전통 한과 세트", benefit: "기력 보충", desc: "수제 유과, 약과 5종 구성" },
    ]
  },
  "2": {
    month: "2월",
    ...
    memoryBox: {
      title: "2월 추억상자",
      item: "쫀득이 찹쌀떡",
      story: "설날 하면 떠오르는 쫀득한 찹쌀떡! 이번 추억상자에는 전통 방식으로 만든 '쫀득이' 찹쌀떡을 넣어드렸어요. 80년 전통 떡집에서 직접 빚은 찹쌀떡으로, 어린 시절 명절마다 할머니가 해주시던 그 맛을 담았습니다. 콩고물의 고소함과 찹쌀의 쫀득함이 어우러져, 온 가족이 함께 나누기 좋아요."
    },
    products: [
      { name: "1++ 한우 등심 300g", benefit: "고단백 철분", desc: "횡성한우 1++ 등급, 28일 숙성" },
      { name: "1++ 한우 안심 200g", benefit: "저지방 고단백", desc: "가장 부드러운 부위, 스테이크용" },
      { name: "명절 떡국떡 500g", benefit: "에너지 보충", desc: "국산 쌀 100%, 쫄깃한 식감" },
      { name: "전통 식혜 1L", benefit: "소화 촉진", desc: "무설탕, 국산 엿기름 사용" },
    ]
  },
  "3": {
    month: "3월",
    ...
    products: [
      { name: "지리산 야생화 꿀 500g", benefit: "면역력 증진", desc: "해발 700m 고지대 채취, 비가열 생꿀" },
      { name: "국산 아몬드 150g", benefit: "혈관 건강", desc: "저온 로스팅, 무염 무첨가" },
      { name: "볶음 호두 100g", benefit: "두뇌 건강", desc: "국산 100%, 껍질째 볶음" },
      { name: "캐슈넛 100g", benefit: "뼈 건강", desc: "베트남 직수입, 경화유 無" },
    ]
  }
};
            ...
            <div key={idx} className="mb-8">
              ...
              {section.image && (
                <div className="rounded-lg overflow-hidden mb-4">
                  <img src={giftBoxImage} alt={section.title} className="w-full aspect-video object-cover" />
                </div>
              )}
              ...
              {story.products.map((product, idx) => (
                <div key={idx} className="flex gap-3 pb-4 border-b border-stone-200 last:border-0 last:pb-0">
                  <div className="w-16 h-16 rounded-lg overflow-hidden flex-shrink-0">
                    <img src={giftBoxImage} alt={product.name} className="w-full h-full object-cover" />
                  </div>
